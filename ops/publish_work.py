...
async def main():
    async with async_playwright() as p:
        ...
        # ── STEP 5b: 스타일 적용 (제목 강조/하이라이트, 소제목 fs17+bold, 수치 bold, 요약 bold) ──
        await asyncio.sleep(0.5)
        style_result = await se_frame.evaluate(f"""
            () => {{
                try {{
                    ...
                    const isTitle = (txt) => txt.trim() === '📌 불개미 일일시황 | 2026.04.07 (화)';
                    const isSubhead = (txt) => /^[1-5]️⃣\s/.test(txt.trim());
                    ...
                    paragraphs.forEach(para => {{
                        ...
                        if (/^5️⃣\s요약/.test(text.trim())) afterSummary = true;
                        ...
                        para.nodes.forEach(node => {{
                            ...
                            if (afterSummary && !/^5️⃣\s요약/.test(text.trim()) && text.trim()) {{
                                node.style.bold = true;
                            }}

                            if ((node.value || '').match(numRe)) {{
                                node.style.bold = true;
                            }}
                            numRe.lastIndex = 0;
                        }});
