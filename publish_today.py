...
PASSWORD = "wnFhT9"
LOG_NO_DELETE = None
IMAGE_PATH = "/tmp/openclaw/uploads/daily_blog_20260415.png"
POST_TITLE = "📌 [불개미 일일시황] 2026.04.15 (수)"
POST_TEXT = """📌 불개미 일일시황 | 2026.04.15 (수)
...
5️⃣ 요약
BTC와 ETH 모두 ETF 순유입을 기록하며 기관 자금 유입이 지속되고 있습니다. BTC 미결제약정이 +3.20% 증가하며 레버리지 포지션이 확대되는 모습이며, 코인베이스 프리미엄 +0.008%로 미국 수요가 소폭 우위를 보이고 있습니다. DAT 주간 순유입 +$224M으로 전반적인 시장 유동성이 개선되는 추세입니다."""
        ...
        style_result = await se_frame.evaluate(f"""
            () => {{
                try {{
                    ...
                    const C = 'center';
                    textComp.value = [
                        para('\ud83d\udccc 불개미 일일시황 | 2026.04.15 (수)', C, sBody),
                        para('', C, sBody),
                        para('1\ufe0f\u20e3 BTC ETH 유출입', C, sHead),
                        para('\u2022 BTC: +$358M', C, sBody),
                        para('\u2022 ETH: +$85.19M', C, sBody),
                        para('\u2022 ETF 데이터는 마지막 거래일 기준', C, sBody),
                        para('', C, sBody),
                        para('2\ufe0f\u20e3 미결제약정 추이 (24시간 기준)', C, sHead),
                        para('\u2022 BTC 24시간: +3.20%', C, sBody),
                        para('\u2022 ETH 24시간: +0.59%', C, sBody),
                        para('', C, sBody),
                        para('3\ufe0f\u20e3 DAT 추이', C, sHead),
                        para('\u2022 WEEKLY NET INFLOW: +$224M', C, sBody),
                        para('', C, sBody),
                        para('4\ufe0f\u20e3 코인베이스 프리미엄', C, sHead),
                        para('\u2022 현재 지수: +0.0082%', C, sBody),
                        para('', C, sBody),
                        para('5\ufe0f\u20e3 요약', C, sSumH),
                        para('BTC와 ETH 모두 ETF 순유입을 기록하며 기관 자금 유입이 지속되고 있습니다. BTC 미결제약정이 +3.20% 증가하며 레버리지 포지션이 확대되는 모습이며, 코인베이스 프리미엄 +0.008%로 미국 수요가 소폭 우위를 보이고 있습니다. DAT 주간 순유입 +$224M으로 전반적인 시장 유동성이 개선되는 추세입니다.', C, sSumB),
                        para('', C, sBody),
