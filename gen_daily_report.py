...
- "BTC ({btc_etf})" — 반드시 이 글자 그대로. 절대 누락·변경 금지. 소수점 포함 전체 표기.
- "ETH ({eth_etf})" — 반드시 이 글자 그대로. 절대 누락·변경 금지. 소수점 포함 전체 표기.
- "{dat_now}" — 반드시 이 글자 그대로. 소수점 포함 전체 표기.
- "{btc_oi_24h}" — 반드시 이 글자 그대로.
- "{eth_oi_24h}" — 반드시 이 글자 그대로.
- "{cb_premium}" — 반드시 이 글자 그대로.
숫자 사이에 공백 삽입 금지. "$ 90" "- 0.015" "225.4 8" 등 공백 분리 절대 금지.
단위 M(Million)을 B(Billion)으로 바꾸거나 소수점 자릿수를 줄이는 것 절대 금지.
예: "$225.48M"은 그대로 "$225.48M", "$95.78M"은 그대로 "$95.78M".

어두운 칠판 배경(dark chalkboard green/black)에 분필 스타일로 그려진 암호화폐 일일시황 이미지를 생성해라.

=== 필수 레이아웃: 정확히 2행 2열 그리드 ===
전체 이미지를 다음과 같이 배치해라:
- 상단 헤더 바(전체 너비, 수평 중앙 정렬): "불개미 일일시황" 제목은 이미지 정중앙 상단에 배치. 날짜 {date_str}는 제목 오른쪽 또는 제목 아래 작게.
