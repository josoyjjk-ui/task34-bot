"""
불개미 매매 원칙 개선판 v4 백테스트
BTC 선물 (BTCUSDT), $1,000 초기 시드
기간: 2021-02-28 ~ 2026-02-28 UTC

v3 대비 변경 규칙 (6개):
  F-1. R:R 비율 최소 1:2 강제 (진입 필터)
  F-3. 고점·저점 구조 확인 HH/HL, LH/LL (진입 필터)
  G-1. ATR 기반 트레일링 스탑 (1.5% 고정 → ATR14 동적)
  H-1. 재투자 주기 분기(91일) → 월간(30일)
  H-2. 시드 상한 $5,000 → $10,000
  I-1. 변동성 폭발 구간 진입 금지 (4캔들 중 진폭 3% 이상)
"""
...
MAX_SEED        = 10000.0   # H-2: 시드 상한 $5,000 → $10,000
...
# G-1: ATR 기반 트레일링 (고정 비율 제거, ATR 동적 사용)
ATR_WINDOW      = 14        # ATR14
ATR_MULT        = 1.0       # trail_dist = ATR14 × 1.0 (가격 단위)

# H-1: 월간 재투자 (91일 → 30일)
REINVEST_DAYS   = 30
REINVEST_RATIO  = 0.50

# 역피라미딩 비율 (시드 기준) — D-2 유지
...
# I-1: 변동성 폭발 임계값
VOL_SPIKE_THRESH = 0.03     # 3% 이상 진폭 = 폭발
...
def run_backtest_v4(df):
    ...
    # 완화 조건: 3캔들 중 저점 상승 / 고점 하락
    df["hl_structure"] = df["low_1"] > df["low_3"]
    df["lh_structure"] = df["high_1"] < df["high_3"]

    # I-1: 변동성 폭발 구간 감지 (직전 4캔들)
    for i in range(1, 5):
    ...
    df["vol_spike"] = (
        ...
        (df["candle_range_3"] >= VOL_SPIKE_THRESH) |
        (df["candle_range_4"] >= VOL_SPIKE_THRESH)
    )
    ...
    def open_position(side, price, ts, atr14_val):
        ...
        return {
            ...
            "tp1_done":        False,
            "tp2_done":        False,
            "trail_peak":      price,   # G-1: 트레일링용 피크/트로프
            "trail_dist":      trail_dist,  # G-1: 고정 ATR 거리
    ...
    for row in rows:
        ...
        # ── 포지션 있음 ────────────────────────────────────────────────────────
            ...
            side = pos["side"]
            params = seed_params(seed)
            trail_dist = pos["trail_dist"]  # G-1: 진입 시점 고정 ATR 거리
            ...
            if not pos["tp1_done"]:
                ...
                if tp1_hit:
                    ...
                    pos["trail_peak"] = close  # 트레일링 기준점 초기화
...
# ── 결과 분석 ─────────────────────────────────────────────────────────────────
def analyze_v4(trades, pnl_cum, max_dd, equity_peak, final_seed):
    if not trades:
        return "거래 없음", None
...
def build_report(v4):
    ...
    lines = []
    lines.append("=" * 76)
    lines.append("   백테스트 비교: 기존 vs v2 vs v3 vs v4")
    lines.append("   기간: 2021-02-28 ~ 2026-02-28 | 초기시드: $1,000 | 5x")
    lines.append("=" * 76)
    lines.append(f"{'항목':<22} {'기존':>11} {'v2':>11} {'v3':>11} {'v4':>11}")
    lines.append("─" * 76)
    lines.append(f"{'거래횟수':<22} {o['total_trades']:>9,}회 {v2['total_trades']:>9,}회 {v3['total_trades']:>9,}회 {v4r['total_trades']:>9,}회")
    lines.append(f"{'승률':<22} {o['win_rate']:>9.1f}% {v2['win_rate']:>9.1f}% {v3['win_rate']:>9.1f}% {v4r['win_rate']:>9.1f}%")
    lines.append(f"{'강제청산':<22} {o['liq_cnt']:>10}회 {v2['liq_cnt']:>10}회 {v3['liq_cnt']:>10}회 {int(v4r['liq_cnt']):>10}회")
    lines.append("─" * 76)
    ...
    for yr in yrs:
        ...
        v4_val = v4r["yearly"].get(yr, 0)
        lines.append(f"{str(yr) + ' 손익':<22} {fmt_pnl(o_val):>11} {fmt_pnl(v2_val):>11} {fmt_pnl(v3_val):>11} {fmt_pnl(v4_val):>11}")

    lines.append("─" * 76)
    lines.append(f"{'누적수익':<22} {fmt_pnl(o['pnl_cum']):>11} {fmt_pnl(v2['pnl_cum']):>11} {fmt_pnl(v3['pnl_cum']):>11} {fmt_pnl(v4r['pnl_cum']):>11}")
    v3_seed_str = "$" + f"{v3.get('final_seed', 1000):,.0f}"
    v4_seed_str = "$" + f"{v4r['final_seed']:,.0f}"
    lines.append(f"{'최종시드':<22} {'$1,000':>11} {'$1,000':>11} {v3_seed_str:>11} {v4_seed_str:>11}")
    o_eq  = "$" + f"{o['final_equity']:,.0f}"
    v2_eq = "$" + f"{v2['final_equity']:,.0f}"
    v3_eq = "$" + f"{v3['final_equity']:,.0f}"
    v4_eq = "$" + f"{v4r['final_equity']:,.0f}"
    lines.append(f"{'최종자산':<22} {o_eq:>11} {v2_eq:>11} {v3_eq:>11} {v4_eq:>11}")
    lines.append(f"{'MDD':<22} {o['max_dd']:>9.1f}% {v2['max_dd']:>9.1f}% {v3['max_dd']:>9.1f}% {v4r['max_dd']:>9.1f}%")
    lines.append("=" * 76)
...
if __name__ == "__main__":
    ...
    trades_v4, pnl_v4, mdd_v4, peak_v4, final_seed_v4 = run_backtest_v4(df)
    v4_stats = analyze_v4(trades_v4, pnl_v4, mdd_v4, peak_v4, final_seed_v4)

    if isinstance(v4_stats, str):
        print(v4_stats)
    else:
        yearly_dict = {int(yr): float(pnl)
                       for yr, pnl in v4_stats["yearly"].items()}
        v4_for_report = {
            "total_trades": v4_stats["total_trades"],
            "win_rate":     v4_stats["win_rate"],
            "liq_cnt":      int(v4_stats["liq_cnt"]),
            "yearly":       yearly_dict,
            "pnl_cum":      v4_stats["pnl_cum"],
            "final_seed":   v4_stats["final_seed"],
            "final_equity": v4_stats["final_equity"],
            "max_dd":       v4_stats["max_dd"],
        }

        report = build_report(v4_for_report)
        print("\n" + report)

        out_path = "/Users/fireant/.openclaw/workspace/backtest_result_v4.txt"
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(report)
            f.write("\n\n[v4 거래 내역 (최근 50건)]\n")
            f.write(v4_stats["df_trades"].tail(50).to_string())
        print(f"\n결과 저장 완료: {out_path}")
