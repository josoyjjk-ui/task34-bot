...
def send_message(chat_id, text):
    ...
    try:
        ...
        if data.get("ok"):
            ...
            if "blocked" in err.lower() or "bot was blocked" in err.lower() or "user is deactivated" in err.lower() or "chat not found" in err.lower() or "forbidden" in err.lower():
                return "blocked"
            return f"fail:{err}"
...
def main():
    ...
    print(f"[공병] 브로드캐스트 시작 — 총 {total}명")
    sys.stdout.flush()
    ...
    for i, (user_id,) in enumerate(rows, 1):
        ...
        if i % 100 == 0:
            print(f"  [{i}/{total}] 성공:{success} 차단:{blocked} 실패:{failed}")
            sys.stdout.flush()
